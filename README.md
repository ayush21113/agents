...
Start the agent: python multi-user-transcriber.py start

Join room via LiveKit Playground

Test scenarios while agent is speaking:

Say filler words - should be ignored

Say stop commands - should interrupt immediately

Test when agent is quiet:
...
Language specific: Currently optimized for English filler patterns

Confidence thresholds: May need tuning for specific audio environments

TTS interruption: While detection works reliably, TTS cancellation may have brief latency
...
Quick Start
Setup Environment: Set API keys and environment variables

Start Server: Terminal 1 - LiveKit server

Run Agent: Terminal 2 - Python agent with PYTHONPATH set

Create Room: Terminal 3 - Generate room and tokens

Test: Use LiveKit Playground to join and test
